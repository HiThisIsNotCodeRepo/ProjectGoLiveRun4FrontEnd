...
import { TasksListComponent } from 'app/modules/apps/post-task/list/list.component';
import { PostTaskService } from 'app/modules/apps/post-task/post-task.service';
...
export class TasksDetailsComponent implements OnInit, AfterViewInit, OnDestroy
{
    ...
    constructor(
        ...
        private _tasksService: PostTaskService,
    ...
    ngOnInit(): void
    {
        ...
        this._tasksService.task$
            ...
            .subscribe((task: Task) => {
                ...
                // Patch values to the form from the browse-task
        ...
        // Update browse-task when there is a value change on the browse-task form
        this.taskForm.valueChanges
            .pipe(
                tap((value) => {

                    // Update the browse-task object
            ...
            .subscribe((value) => {
                ...
                // Update the browse-task on the server
    ...
    filterTagsInputKeyDown(event): void
    {
        ...
        // If the found tag is already applied to the browse-task...
        if ( isTagApplied )
        {
            // Remove the tag from the browse-task
            this.deleteTagFromTask(tag);
        }
        else
        {
            // Otherwise add the tag to the browse-task
    ...
    createTag(title: string): void
    {
        ...
        this._tasksService.createTag(tag)
            .subscribe((response) => {
                ...
                // Add the tag to the browse-task
    ...
    /**
     * Delete tag from the browse-task
     *
     * @param tag
     */
    ...
    /**
     * Check if the browse-task is overdue or not
     */
    isOverdue(): boolean
    {
    ...
    deleteTask(): void
    {
        // Get the current browse-task's id
        const id = this.task.id;

        // Get the next/previous browse-task's id
        ...
        this._tasksService.deleteTask(id)
            .subscribe((isDeleted) => {
                ...
                // Return if the browse-task wasn't deleted...
                if ( !isDeleted )
                {
                    return;
                }

                // Navigate to the next browse-task if available
                if ( nextTaskId )
                {
